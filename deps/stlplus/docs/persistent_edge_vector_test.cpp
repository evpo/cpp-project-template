...
void print_edge (std::ostream& str, const edge& e)
{
  str << e;
}
...
std::ostream& operator<< (std::ostream& str, const edge_vector& e)
{
  stlplus::print_vector(str, e, print_edge, std::string("*"));
  return str;
}

void dump_edge_vector(stlplus::dump_context& context, const edge_vector& e)
{
  stlplus::dump_vector(context, e, dump_edge);
}

void restore_edge_vector(stlplus::restore_context& context, edge_vector& e)
{
...
#define DUMP "edge_vector_test.tmp"
#define MASTER "edge_vector_test.dump"

int main(int argc, char* argv[])
{
  bool result = true;
  std::cerr << stlplus::build() << " testing edge vector" << std::endl;

  try
  {
    // create an edge vector
    edge_vector data;
    data.push_back(edge(point(0,0,0),point(0,0,1)));
    data.push_back(edge(point(0,0,1),point(0,1,1)));
    data.push_back(edge(point(0,1,1),point(0,1,0)));
    data.push_back(edge(point(0,1,0),point(0,0,0)));

    std::cerr << data << std::endl;

    // now dump it
    std::cerr << "dumping" << std::endl;
    stlplus::dump_to_file(data,DUMP,dump_edge_vector,0);

    // now restore a copy and compare it
    std::cerr << "restoring" << std::endl;
    edge_vector restored;
    stlplus::restore_from_file(DUMP,restored,restore_edge_vector,0);
    ...
    else
    {
      ...
      edge_vector master;
      stlplus::restore_from_file(MASTER,master,restore_edge_vector,0);
      if (data == master)
